...
extern "C" {
    fn fastnn_cuda_init(device_id: i32) -> i32;
    fn fastnn_cuda_device_count() -> i32;
    fn fastnn_cuda_synchronize() -> i32;
    fn fastnn_cuda_get_memory_info(free: *mut usize, total: *mut usize);
}
...
impl CudaContext {
    ...
    pub fn new(device_id: i32) -> Result<Self, String> {
        let mut result = Ok(());
        CUDA_INIT.call_once(|| {
            let ret = unsafe { fastnn_cuda_init(device_id) };
            if ret != 0 {
    ...
    pub fn device_count() -> i32 {
        unsafe { fastnn_cuda_device_count() }
    }

    pub fn synchronize() -> Result<(), String> {
        let ret = unsafe { fastnn_cuda_synchronize() };
        if ret != 0 {
    ...
    pub fn memory_info() -> (usize, usize) {
        ...
        unsafe { fastnn_cuda_get_memory_info(&mut free, &mut total) };
        (free, total)
    }
}
